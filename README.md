...
Импорт понимает конфиг sing-box, отдельные share-ссылки (VLESS, VMess,
Trojan, Shadowsocks, Hysteria2, TUIC, SOCKS, HTTP), содержимое подписки в
base64 и ссылку на подписку целиком. Формат не спрашивается, а
определяется: человек копирует то, что ему прислали.

Узел, распознанный, но не способный работать на sing-box (транспорты
XHTTP, mKCP, QUIC от Xray), отвергается с объяснением, а не превращается
молча в TCP-конфиг, который выглядит настроенным и не подключается.

Задержку меряет ядро: оно ходит по ссылке через каждый исходящий. TCP-проба
до адреса узла здесь не годится — половина протоколов из списка живёт
поверх QUIC. Причина отказа возвращается наравне с задержкой: «нет связи»
без объяснения нечинимо.

## Статус

Итерация 1, шаг 7 из 13. Что доделано и что осталось — в [TODO.md](TODO.md).
